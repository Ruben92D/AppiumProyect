...
elementToDrag = wait.until(lambda x: x.find_element(AppiumBy.ID, 'com.code2lead.kwad:id/ingvw'))
elementToDrop = wait.until(lambda x: x.find_element(AppiumBy.ID, 'com.code2lead.kwad:id/layout2'))

actions.long_press(elementToDrag).move_to(elementToDrop).release().perform()
